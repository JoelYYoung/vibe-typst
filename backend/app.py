...
async def _activate_current() -> dict:
    ...
    # If this working dir was already set up, refresh the managed CLAUDE.md section + .mcp.json
...
def setup_workdir():
    """Create/merge .mcp.json + CLAUDE.md in the current working dir (called after the user
...
async def pty_ws(websocket: WebSocket):
    ...
    up the .mcp.json + CLAUDE.md there. NOTE: this is full shell access — don't expose the
    server publicly without auth."""
    await websocket.accept()
    shell = os.environ.get("SHELL", "/bin/bash")
    # Open in the DECK's directory, not HOME. Claude Code keys its sessions to the working
    # directory, so starting here means `claude` picks up this deck's .mcp.json + CLAUDE.md
...
def serve_render(name: str, v: Optional[str] = None):
    if "/" in name or ".." in name:
        raise HTTPException(400, "bad name")
    p = typst_service.render_path(name)
    if not p.exists():
        raise HTTPException(404)
    # `v` is a CONTENT token (hash of these exact bytes). When present, the URL changes iff
    # the content changes, so the response is safely immutable — the browser may reuse it
    # forever without revalidating, and an unchanged page is served from cache with zero
    # network. Without a token (legacy/direct hit) fall back to revalidate-always.
...
@app.post("/api/project/files/duplicate")
async def duplicate_project_file(request: Request):
    """Duplicate a file inside the active project (-> "<name> copy.<ext>")."""
    if not _has_valid_file():
        raise HTTPException(400, "no active project")
    body = await request.json() or {}
    path = (body.get("path") or "").strip()
    if not path:
        raise HTTPException(400, "path is required")
    try:
        return projects_mod.duplicate_file(runtime.project_dir(), path)
    except FileNotFoundError as e:
        raise HTTPException(404, str(e))
...
async def patch(cid: str, request: Request):
    ...
    if not cur:
        raise HTTPException(404)
    if "selections" in fields and "selection" not in fields:
        fields["selection"] = fields.pop("selections")
    # Editing the comment's SELECTION (its anchored elements/pages) must re-enrich page
    # selections and rebuild raw_context from the NEW anchors — otherwise Claude would act
    # on the old context. We also keep `kind`/`page` coherent with the new selection.
    if "selection" in fields:
        sels = fields["selection"] or []
        for s in sels:
            if isinstance(s, dict) and s.get("kind") == "page" and s.get("slide") is None:
                try:
                    si = slidemap.slide_info(s.get("page_no"))
                    ...
                    pass
        body = fields.get("body", cur.get("body", ""))
        fields["raw_context"] = context.build_raw_context(
            {"file": cur.get("file"), "body": body, "selection": sels}, current_source())
        fields["kind"] = "page" if (sels and all(
            isinstance(s, dict) and s.get("kind") == "page" for s in sels)) else "element"
        first_page = next((s.get("page_no") for s in sels
                           if isinstance(s, dict) and s.get("kind") == "page"), None)
        fields["page"] = first_page
    elif "body" in fields and "raw_context" not in fields:
        # Editing only the text: swap the instruction block, keep the captured source snapshot.
