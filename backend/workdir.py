"""Initialize the opened file's directory as an agent working directory:
write/merge Claude and Codex MCP config plus an auto-managed section in `AGENTS.md`
instructing agents to edit ONLY through our MCP tools (never Read/Edit/Write),
so human + agent edits stay merged in the one live document. `CLAUDE.md` is kept as
a symlink to `AGENTS.md` when that can be done without losing user content.
"""
import json
import os
import time
...
- **MCP** (`vibe-typst` server): defined for Claude in `.mcp.json` and auto-enabled in
  ...
  step. The web backend must be running (its URL is set in those config files).
...
def _merge_managed(existing: str, section: str, begin: str, end: str) -> str:
...
def _ensure_claude_symlink(d: Path, agents_path: Path) -> None:
    """Make CLAUDE.md point at AGENTS.md, preserving any user-authored old file."""
    ...
    try:
        ...
        if unmanaged:
            ...
            backup = d / f"CLAUDE.md.backup-{int(time.time())}"
...
def setup(backend_port: int | None = None) -> dict:
    ...
    # --- .codex/config.toml: project-scoped Codex MCP config ---
    codex_path = d / ".codex" / "config.toml"
    try:
