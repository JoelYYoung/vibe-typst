...
def create_file(project_dir: Path, name: str) -> dict:
    """Create a new empty file inside the project (path may include subdirs).

    Any extension is allowed (e.g. refs.bib, data.json, notes.md). Only when the name has NO
    extension do we default to .typ, since this is a Typst project.
    """
    ...
    return {"path": rel, "name": target.name, "size": 0, "is_typ": target.suffix == ".typ",
            "type": "file", "abs_path": str(target.resolve())}
...
def duplicate_file(project_dir: Path, rel_path: str) -> dict:
    """Copy a file to a sibling named "<stem> copy<suffix>" (then "copy 2", … if taken)."""
    src = _resolve_project_path(project_dir, rel_path)
    if not src.is_file():
        raise FileNotFoundError(f"{rel_path!r} not found")
    cand = src.parent / f"{src.stem} copy{src.suffix}"
    i = 2
    while cand.exists():
        cand = src.parent / f"{src.stem} copy {i}{src.suffix}"
        i += 1
    shutil.copy2(src, cand)
    rel = str(cand.relative_to(project_dir))
    return {"path": rel, "name": cand.name, "size": cand.stat().st_size,
            "is_typ": cand.suffix == ".typ", "type": "file", "abs_path": str(cand.resolve())}
